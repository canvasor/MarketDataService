...
其次支持 CoinMarketCap Pro。Fear & Greed 继续走免费接口。
"""
...
class CMCCollector:
    ...
    async def _request_json(self, url: str, headers: Optional[Dict[str, str]] = None, params: Optional[Dict] = None) -> Any:
        ...
        try:
            async with session.get(url, headers=merged_headers, params=params, timeout=15) as resp:
                if resp.status != 200:
                    logger.warning("Market API error %s: %s", resp.status, await resp.text())
                    return {}
    ...
    async def _request_coingecko(self, endpoint: str, params: Optional[Dict] = None) -> Any:
        ...
        return await self._request_json(f"{self.coingecko_api_endpoint}{endpoint}", headers=headers, params=params)

    async def get_latest_listings(self, limit: int = 200) -> Dict[str, CMCCoinData]:
        cache_key = f"listings:{self.active_provider}:{limit}"
        if cache_key in self._market_cache:
            return self._market_cache[cache_key]

        if self.active_provider == "coingecko":
    ...
    async def get_gainers_losers(self, limit: int = 20, time_period: str = "24h") -> tuple:
        cache_key = f"gainers_losers:{self.active_provider}:{limit}:{time_period}"
    ...
    async def get_trending(self, limit: int = 20) -> List[CMCTrendingData]:
        cache_key = f"trending:{self.active_provider}:{limit}"
        if cache_key in self._trending_cache:
            return self._trending_cache[cache_key]
            ...
            for row in coins[:limit]:
                ...
                results.append(
                    CMCTrendingData(
                        ...
                        name=item.get("name", ""),
                        cmc_rank=int(item.get("market_cap_rank") or 0),
                        price=float(data_obj.get("price") or 0),
                        percent_change_24h=float((data_obj.get("price_change_percentage_24h") or {}).get("usd") or 0),
                        volume_change_24h=0.0,
                        trending_score=max(0.0, 100 - float(item.get("market_cap_rank") or 100)),
                    )
                )
    ...
    async def get_market_overview(self) -> Dict:
        cache_key = f"market_overview:{self.active_provider}"
        if cache_key in self._market_cache:
            return self._market_cache[cache_key]
