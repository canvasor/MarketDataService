...
@app.get("/api/system/strategy-universe")
async def get_strategy_universe(auth: str = Query(..., description="认证密钥")):
    if not verify_auth(auth):
        raise HTTPException(status_code=401, detail="认证失败")
...
@app.get("/api/system/nofx-adaptation-checklist")
async def get_nofx_adaptation_checklist(auth: str = Query(..., description="认证密钥")):
    if not verify_auth(auth):
        raise HTTPException(status_code=401, detail="认证失败")
    return {
        "success": True,
...
@app.get("/api/strategy/pair-neutral/template")
async def get_pair_neutral_template(auth: str = Query(..., description="认证密钥")):
    if not verify_auth(auth):
        raise HTTPException(status_code=401, detail="认证失败")
    return {
        "success": True,
...
@app.get("/api/strategy/pair-neutral/context")
async def get_pair_neutral_context(
    ...
    if not verify_auth(auth):
        raise HTTPException(status_code=401, detail="认证失败")
    data = await build_pair_neutral_context(collector, symbol_a, symbol_b, lookback_bars=lookback_bars, interval=interval)
    return {"success": True, "data": data}
